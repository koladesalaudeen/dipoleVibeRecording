...
const extractUsernameFromEmail =
  require("../utils/emailSplit").extractUsernameFromEmail;
  ...
  try {
    ...
    if (email) {
      ...
      if (!user.isPaidUser) {
        return res
          .status(403)
          .json({ error: "You are not a paid user. Cannot update comment." });
      }
    }

    if (user && comment.userId.toString() !== user._id.toString()) {
      return res.status(403).json({
        error: "You are not the author of this comment. Cannot update.",
      });
    }
  ...
  try {
    ...
    if (email) {
      ...
      if (!user.isPaidUser) {
        return res
          .status(403)
          .json({ error: "You are not a paid user. Cannot delete comment." });
      }
    }

    if (user && comment.userId.toString() !== user._id.toString()) {
      return res.status(403).json({
        error: "You are not the author of this comment. Cannot delete.",
      });
    }
